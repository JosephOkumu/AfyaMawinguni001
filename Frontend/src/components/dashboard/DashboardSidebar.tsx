    ...
    <Sidebar side="left" variant="sidebar" collapsible="icon">
      <SidebarHeader className="py-5 bg-gradient-to-br from-secondary-green to-secondary-green/80">
        <div className="flex flex-col items-center justify-center px-4">
          ...
          <span className="mt-2 font-medium text-sm text-white">John Doe</span>
      ...
      <SidebarContent className="bg-gradient-to-b from-green-50 to-white">
        <SidebarMenu>
          {mainMenuItems.map((item) => (
            <SidebarMenuItem key={item.title}>
              ...
              >
                <Link to={item.path}>
                  <item.icon className={`h-5 w-5 ${item.isActive ? 'text-secondary-green' : 'text-gray-500'}`} />
        ...
        <div className="mt-8">
          <div className="px-4 mb-2 text-xs font-medium text-secondary-green/70">USER</div>
          <SidebarMenu>
      ...
      <SidebarFooter className="bg-white border-t border-gray-100">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild tooltip="Log Out" className="hover:bg-red-50 hover:text-red-500">
